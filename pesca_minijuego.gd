...
@export var max_game_time := 7.0
@export var velocidad_garra := 200.0
...
func _process(delta):
	...
	if movimiento != Vector2.ZERO:
		...
		gato.position += movimiento * velocidad_garra * delta

	# Evita que la garra salga del área del panel
	gato.position.x = clamp(gato.position.x, 0.0, 300.0)
	gato.position.y = clamp(gato.position.y, 0.0, 260.0)
...
func _input(event):
	...
	# ESPACIO = intentar atrapar el pez
...
func _check_fish_click():
	var gato_pos = gato.global_position
	var fish_pos = pez_sprite.global_position

	if gato_pos.distance_to(fish_pos) < 25:
		_fish_caught()
...
func _game_over():
	...
	# Cierra automáticamente después de 1 segundo
	await get_tree().create_timer(1.0).timeout
